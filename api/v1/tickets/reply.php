...
if (isset($_POST['ticket_reply_time_worked']) && preg_match('/^\d{1,3}:[0-5]\d:[0-5]\d$/', $_POST['ticket_reply_time_worked'])) {
    $ticket_reply_time_worked = escapeSql($_POST['ticket_reply_time_worked']);
} else {
...
if (!empty($ticket_id) && !empty($ticket_reply)) {
    ...
    if ($ticket_row) {
        ...
        if ($insert_sql) {
            ...
            // Logging
            logAudit("Ticket", "Reply", "$ticket_prefix$ticket_number ticket via API ($api_key_name) and was a $ticket_reply_type reply", $client_id, $ticket_id);
            logAudit("API", "Success", "Replied to ticket $ticket_prefix$ticket_number via API ($api_key_name)", $client_id);
        }
    }
}
